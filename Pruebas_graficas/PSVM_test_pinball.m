...
[w_lp, b_lp, ~, ~] = psvm_primal(X, y, C, epsilon,tau);
% f(x) = w'*x + b - 0.5
...
for k = 1:4
    ...
    [w_pts, b_pts,f_vals,pred_vals] = psvm_dual_quadprog_pinball(X, y, kt, C, epsilon,d,sigma,tau,gridPts);
    [w_score, b_score,f_score,pred_score] = psvm_dual_quadprog_pinball(X, y, kt,C,epsilon,d,sigma,tau,X);
