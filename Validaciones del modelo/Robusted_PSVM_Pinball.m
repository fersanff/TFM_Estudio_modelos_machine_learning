...
dataset_file = datasets{2,1};   % selecciona breastcancer
dataset_name = datasets{2,2};
load(dataset_file);
[m, n] = size(X);
fInd    = 1:n;
CV      = 10;
...
[param_u, ~, ~ , ~] = psvm_dual_quadprog_pinball(...
    Xtr, Ytr, kerneltype, C, epsilon, d, sigma,tau, Xt);
...
for i = 1:T
    ...
    [param_i, ~, ~, ~] = psvm_dual_quadprog_pinball(...
        Xtr, Ytr, kerneltype, C, epsilon, d, sigma,tau, Xt);
