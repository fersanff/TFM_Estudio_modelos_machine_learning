...
dataset_file = datasets{3,1};   % selecciona breastcancer
dataset_name = datasets{3,2};
load(dataset_file);
[m, n] = size(X);
fInd    = 1:n;
CV      = 10;
...
[param_u, ~, ~, ~] = svm_soft_margin_quadprog(...
     Xtr, Ytr, kerneltype, tau,C, sigma, d, Xt);
...
for i = 1:T
    ...
    [param_i, ~, ~ , ~] = svm_soft_margin_quadprog(...
        Xtr, Ytr, kerneltype, tau,C, sigma, d, Xt);
